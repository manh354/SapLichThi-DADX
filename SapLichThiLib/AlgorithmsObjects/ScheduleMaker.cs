...
namespace SapLichThiLib.AlgorithmsObjects
{
    ...
    public class ScheduleMaker
    {
        ...
        public ScheduleMaker(Dictionary<int, HashSet<StudyClass>> colorSetOfCommonClassesPairs, Dictionary<Course, HashSet<StudyClass>> commonCourse_Class_Dictionary, Dictionary<int, HashSet<StudyClass>> colorSetOfNonCommonClassesPairs,Dictionary<StudyClass, List<ExamClass>> studyClassExamClassPairs, Dictionary<School, HashSet<StudyClass>> school_StudyClassPairs, List<Room> roomList, List<DateOnly> dateList,List<int> shiftList, float increaseThreshold)
        {
            Color_SetOfCommonClasses_Dictionary = colorSetOfCommonClassesPairs;
            CommonCourse_Class_Dictionary = commonCourse_Class_Dictionary;
            Color_SetOfNonCommonClasses_Dictionary = colorSetOfNonCommonClassesPairs;
            StudyClass_ExamClass_Dictionary = studyClassExamClassPairs;
            School_StudyClass_Ditcionary = school_StudyClassPairs;
            RoomList = roomList;
            DateList = dateList;
            ShiftList = shiftList;
        ...
        {
            O_schedule = new ExamSchedule(DateList.ToArray(), ShiftList.ToArray(), RoomList.ToArray());
        ...
        void FitOneCommonCourseIntoSchedule(KeyValuePair<Course, HashSet<StudyClass>> classesOfOneCourse)
        {
            ...
            while (classesOfOneCourse.Value.Count > 0)
            {
                ...
                if (mode == fitMode.threeQuater)
                    ...
                    capacityMul = 0.5f;
                Logger.logger.LogMessage($"Phòng nhân {capacityMul}");

                if (!FindSlotForCommonCourses(out int foundDate, out int foundShift))
                {
                    Logger.logger.LogMessage($"Không tìm thấy kíp trống cho môn {classesOfOneCourse.Key.Name}");
                    return;
                }
                Logger.logger.LogMessage($"Tìm thấy ngày {foundDate} - kíp {foundShift} cho môn {classesOfOneCourse.Key.Name}");
                ...
                {
                    Logger.logger.LogMessage($"Lớp {studyClass.ID} có {studyClass.Count} sinh viên");
                    ...
                    if (FindBestRoomToFitCommonStudyClass(foundDate, foundShift, studyClass,capacityMul, out var bestRoom))
                    {
                        ...
                        Logger.logger.LogMessage($"Tìm thấy phòng {bestRoom.RoomId} sức chứa {bestRoom.Capacity} cho lớp {studyClass.ID} SLDK {studyClass.Count} môn {studyClass.Course.Name}");
                        classesOfOneCourse.Value.Remove(studyClass);
                        continue;
                    }
                    Logger.logger.LogMessage($"Không tìm thấy phòng cho lớp {studyClass.ID} môn {studyClass.Course.Name}");
                    break;
                }
                foreach (var studyClass in classesOfOneCourse.Value)
                {
                    foreach (var examClass in StudyClass_ExamClass_Dictionary[studyClass])
                    {
                        ...
                        Logger.logger.LogMessage($"Lớp exam của {studyClass.ID} có {examClass.Count} sinh viên");
                        if(FindBestRoomToFitCommonExamClass(foundDate, foundShift, examClass, capacityMul, out var bestRoom))
                        {
                            ...
                            Logger.logger.LogMessage($"Tìm thấy phòng {bestRoom.RoomId} sức chứa {bestRoom.Capacity} cho lớp {studyClass.ID} SLDK {examClass.Count} môn {studyClass.Course.Name}");
                            classesOfOneCourse.Value.Remove(studyClass);
                            continue;
                        }
                        Logger.logger.LogMessage($"Không tìm thấy phòng cho lớp exam {studyClass.ID}");
                        break;
                    }
                }
            }
