...
{
    public class SingleExamClassMove : INeighborhoodMove
    {
        ...
        private SingleClassMoveDetails? P_singleCourseMoveDetails { get; set; } = null;
        private ClassMoveInfos? P_classMoveInfos { get; set; } = null;

        bool INeighborhoodMove.CreateNeighbor()
        {
            (P_singleCourseMoveDetails, P_classMoveInfos) = CreateNeighbor();
            return P_singleCourseMoveDetails != null;
        }

        (SingleClassMoveDetails?, ClassMoveInfos?) CreateNeighbor()
        {
            // Logic to find a single exam class
            var chosenExamClass = RandomExtension.PickRandomFromList(I_examClasses);
            var available = FindValidDestinationSlots(chosenExamClass, P_exam_slot[chosenExamClass], out var allPossiblePonds);

            if (!available)
                return (null, null);
            var oldPond = I_lake.GetPond(P_exam_slot[chosenExamClass].First());
            var newPond = RandomExtension.PickRandomFromList(allPossiblePonds);

            var availableRooms = FindValidDestinationRooms(chosenExamClass, newPond, out var allPossiblePuddles);
            if (!availableRooms)
                return (null, null);
            ...
            var newPosition = (newPond.Period, newPuddle.Room);
            var singleCourseMoveDetails = new SingleClassMoveDetails()
            {
            ...
            ReadPondInfoStartPosition(classMoveInfos, chosenExamClass, oldPond, newPuddle);
            UpdateLake(singleCourseMoveDetails, false);
            UpdateMaps(singleCourseMoveDetails, false);
            ReadPondInfoEndPosition(classMoveInfos, chosenExamClass, newPond, newPuddle);

            return (singleCourseMoveDetails, classMoveInfos);
        }
        ...
        {
            RevertNeighbor(P_singleCourseMoveDetails, P_classMoveInfos);
        }

        void RevertNeighbor(SingleClassMoveDetails singleCourseMoveInfo, ClassMoveInfos classMoveInfos)
        {
        ...
        {
            P_old_exam_slot[P_singleCourseMoveDetails.ExamClass] = P_exam_slot[P_singleCourseMoveDetails.ExamClass];
        }

        void UpdateLake(SingleClassMoveDetails singleCourseMoveInfo, bool revert)
        {
        ...
        void UpdateMaps(SingleClassMoveDetails singleCourseMoveInfo, bool revert)
        {
            var startPosition = revert ? singleCourseMoveInfo.NewPosition : singleCourseMoveInfo.OldPosition;
            var endPosition = revert ? singleCourseMoveInfo.OldPosition : singleCourseMoveInfo.NewPosition;
            var examClass = singleCourseMoveInfo.ExamClass;
        ...
        public class SingleClassMoveDetails
        {
